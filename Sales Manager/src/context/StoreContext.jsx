...
export default function StoreContextProvider({ children }) {
  ...
  const [customers, setCustomers] = useState(() => {
    ...
    return [
      { id: 'target-fitness-01', name: 'Mr Sidath Rathnayake', gymName: 'Target Fitness', email: '', phone: '0777888637', purchaseDate: '2026-02-24', renewalDate: '2027-02-24', annualFee: 70000, status: 'Active', notes: [] },
      { id: uuidv4(), name: 'John Doe', gymName: 'FitLife Gym', email: 'john@fitlife.com', phone: '0712345678', purchaseDate: '2023-05-15', renewalDate: '2024-05-15', annualFee: 1200, status: 'Active', notes: [] },
    ];
  });
  ...
  const convertQuoteToInvoice = (quoteId) => {
    ...
    let matchedCustomer = customers.find(c => 
      (c.phone && c.phone === quote.prospectPhone) || 
      (c.gymName && c.gymName === quote.prospectName)
    );
    
    let targetCustomerId = matchedCustomer?.id;
    if (!targetCustomerId) {
      targetCustomerId = uuidv4();
      const newCustomer = { 
        id: targetCustomerId,
        gymName: quote.prospectName || 'Unknown Gym',
        name: 'Gym Contact',
      ...
      addLog('System', `Auto-created Gym from Quote: ${quote.prospectName}`);
    }
    ...
    showNotification('Quote successfully converted to Draft Invoice!', 'success');
  };
  ...
  const triggerSMS = (type, customer, documentData) => {
    ...
    msg = template
      ...
      .replace(/{gym}/g, cGym)
      .replace(/{amount}/g, documentData?.amount || customer?.annualFee || 0)
      ...
      .replace(/{link}/g, documentData?.id ? `${window.location.origin}/share/${type.toLowerCase() === 'quotation' ? 'quote' : 'invoice'}/${documentData.id}` : '');
